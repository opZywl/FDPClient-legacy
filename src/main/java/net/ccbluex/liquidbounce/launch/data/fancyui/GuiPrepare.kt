...
class GuiPrepare(private val progressHandler: DynamicProgressHandler, private val doneCallback: () -> Unit) :
    GuiScreen() {
    ...
    override fun drawScreen(p_drawScreen_1_: Int, p_drawScreen_2_: Int, p_drawScreen_3_: Float) {
        ...
        // alert text
        fontRendererObj.drawCenteredString(
            "Please wait while we are preparing the renderer...",
            width / 2f,
            height / 2f - 50 - fontRendererObj.FONT_HEIGHT,
            0xffffff
        )
        fontRendererObj.drawCenteredString(
            "Current stage: ${progressHandler.currentProgressText}",
            width / 2f,
            height / 2f - 50,
            0xffffff
        )
        ...
        RenderUtils.drawRect(
            (barStart).coerceAtLeast(0f).coerceAtMost(1f) * barWidth, 0f,
    ...
    class DynamicProgressHandler : ConsoleProgressHandler() {
        ...
        private fun updateProgressText() {
            currentProgressText =
                currentProgress.name.substring(0, 1).uppercase() + currentProgress.name.substring(1).lowercase()
        }
    }
}