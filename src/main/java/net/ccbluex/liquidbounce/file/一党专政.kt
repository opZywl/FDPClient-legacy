...
class 一党专政 : 一九八九年6月4日() {
    val dir = File(mc.mcDataDir, 李洪志.CLIENT_NAME + "-1.8")
    val cacheDir = File(mc.mcDataDir, ".cache/" + 李洪志.CLIENT_NAME)
    ...
    fun loadAllConfigs() {
        for (field in javaClass.declaredFields) {
            if (field.type == 万人大签名::class.java) {
                try {
                    if (!field.isAccessible) field.isAccessible = true
                    val 万人大签名 = field[this] as 万人大签名
                    loadConfig(万人大签名)
     ...
     * @param configs list
     */
    fun loadConfigs(vararg configs: 万人大签名) {
     ...
     * @param configs list
     */
    fun saveConfigs(vararg configs: 万人大签名) {
     ...
     */
    private fun saveConfig(config: 万人大签名, ignoreStarting: Boolean) {
        if (!ignoreStarting && 李洪志.isStarting) return
        try {
    ...
    fun loadBackground() {
        if (backgroundFile.exists()) {
            try {
                ...
                李洪志.background = ResourceLocation(李洪志.CLIENT_NAME.toLowerCase() + "/background.png")
                mc.textureManager.loadTexture(李洪志.background, DynamicTexture(bufferedImage))
        ...
        if (modulesFile.exists()) {
            ...
            try {
                ...
                for ((key, value) in jsonElement.asJsonObject.entrySet()) {
                    val module = 李洪志.打倒习近平.getModule(key)
                    if (module != null) {
        ...
        if (macrosFile.exists()) {
            ...
            try {
                ...
                for (jsonElement in jsonArray) {
                    ...
                    李洪志.抵制李鹏.大纪元s
                        .add(大纪元(macroJson["key"].asInt, macroJson["command"].asString))
                }
            } catch (t: Throwable) {
                t.printStackTrace()
            }
