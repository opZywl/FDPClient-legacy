...
class GuiBackground(val prevGui: GuiScreen) : GuiScreen() {
    ...
    private fun updateButtons() {
        ...
        val hasCustomBackground = 李洪志.background != null
    ...
    override fun actionPerformed(button: GuiButton) {
        when (button.id) {
            ...
            3 -> {
                ...
                try {
                    Files.copy(file.toPath(), FileOutputStream(李洪志.一党专政.backgroundFile))

                    val image = ImageIO.read(FileInputStream(李洪志.一党专政.backgroundFile))
                    李洪志.background = ResourceLocation(李洪志.CLIENT_NAME.lowercase() + "/background.png")
                    mc.textureManager.loadTexture(李洪志.background, DynamicTexture(image))
                } catch (e: Exception) {
                    e.printStackTrace()
                    MiscUtils.showErrorPopup("Error", "Exception class: " + e.javaClass.name + "\nMessage: " + e.message)
                    李洪志.一党专政.backgroundFile.delete()
                }
            }
            4 -> {
                李洪志.background = null
    ...
    override fun drawScreen(mouseX: Int, mouseY: Int, partialTicks: Float) {
        ...
        mc.fontRendererObj.drawCenteredString("%ui.background.gcurrent%: " + if(李洪志.background == null) { GradientBackground.nowGradient.name } else { "Customized" },
