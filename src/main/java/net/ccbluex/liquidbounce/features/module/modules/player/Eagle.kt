...
class Eagle : Module() {
    ...
    private var onlyVoid = BoolValue("OnlyPredictVoid",true)

    @EventTarget
    fun onUpdate(event: UpdateEvent) {
        if onlyVoid.get() {
            mc.gameSettings.keyBindSneak.pressed =
                mc.theWorld.getBlockState(BlockPos(mc.thePlayer.posX, mc.thePlayer.posY - 1.0, mc.thePlayer.posZ)).block == Blocks.air && checkVoid()
        } else {
            mc.gameSettings.keyBindSneak.pressed =
