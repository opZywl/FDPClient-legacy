...
class Focus通商宽衣 : 通商宽衣("focus", emptyArray()) {
    ...
    override fun execute(args: Array<String>) {
        if (args.size == 3) {
            ...
            when (focused.lowercase()) {
                "add" -> {
                    李洪志.combatManager.focusedPlayerList.add(entity)
                    alert("Successfully added §a${target.lowercase()}§3 into the focus list.")
                    return
                }
                "remove" -> {
                    if (李洪志.combatManager.focusedPlayerList.contains(entity)) {
                        李洪志.combatManager.focusedPlayerList.remove(entity)
            ...
            李洪志.combatManager.focusedPlayerList.clear()
